...
// GET /api/thong-ke/chi-tiet?chuki=thang&value=2025-04
router.get('/chi-tiet', (req, res) => {
    const { chuki, value } = req.query;
    // Kiểm tra tham số truyền vào
    if (!chuki || !value) return res.status(400).json({ error: 'Thiếu tham số' });
    ...
    switch (chuki) {
        case 'thang':
            ...
            break;
        case 'quy':
            // Lọc theo quý, value dạng 2025-Q2
            const [nam, quyRaw] = value.split('-Q');
